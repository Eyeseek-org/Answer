...
import { getLatestBlockHeight, getLogEvents} from '../../pages/api/myTx';
...
const MyTxTable = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const logPolyEvents = await getLogEvents(startingPolyBlock, 80001, polyBlock, address);
      const logBnbEvents = await getLogEvents(startingBnbBlock, 97, bnbBlock, address);
      const logFtmEvents = await getLogEvents(startingFtmBlock, 4002, ftmBlock, address);
