...
const RewardCreate = ({ objectId, bookmarks, home, pid }) => {
  ...
  const handleCreateReward = async () => {
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_DAPP}/classes/Reward`,
        {
          title: rewardTitle,
      ...
      <RewardContainer>
        <TabRow>
          <Tab
            ...
            o3={'ERC1155'}
            change1={() => {
              handleChangeTokenType('Classic');
            }}
            change2={() => {
              handleChangeTokenType('ERC20');
            }}
            change3={() => {
              handleChangeTokenType('ERC1155');
            }}
          />
        </TabRow>
          ...
          <MilestoneContainer>
            ...
            {rType === 'Microfund' && (
              <InputContainer
                ...
                description={
                  <Row>
                    Required amount of backer's microfund
            ...
            <Summary>
              {rType === 'Donate' && (
                <SumRow>
                  <SumTitle>
                    You will receive if fully claimed ={' '}
                    <b>
                      $<Amount value={Number(cap) * Number(pledge)} />
                    </b>
                  </SumTitle>
                </SumRow>
              )}
              {rType === 'Microfund' && (
                <SumRow>
                  <SumTitle>Microfund impact on final collected amount is never same</SumTitle>
                </SumRow>
              )}
              {tokenType === 'ERC20' && (
                <SumRow>
                  <SumTitle>
                    Number of ERC20 you have to lock ={' '}
                    <b>
                      $<Amount value={Number(cap) * Number(tokenAmount)} />
                    </b>
                  </SumTitle>
                </SumRow>
              )}
            ...
            {rewardDesc !== '' ? (
              <>
                {' '}
                {tokenType === 'ERC1155' && (
                  <RewardNftSubmit home={home} pid={pid} cap={cap} tokenAddress={tokenAddress} nftId={nftId} add={add} pledge={pledge} />
                )}
                {tokenType === 'ERC20' && (
                  <RewardTokenSubmit
                    home={home}
                ...
                {!success ? (
                  <>
                    ...
                    {apiError && (
                      ...
                      >
                        Error: Check your fields and retry
                      </NextButton>
                    )}
                  </>
                ) : (
