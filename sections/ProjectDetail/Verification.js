...
const Verification = ({objectId}) => {
    ...
    const handleMoralis = async () => {
        try {
            await axios.post(`${process.env.NEXT_PUBLIC_DAPP}/classes/VeriRequest`, {
                "projectId": objectId,
                "url": url,
            }, moralisApiConfig)
            setApiError(false)
    ...
    return <Container>
         ...
         <div>Send us a proof where we can verify your project is connected with Eyeseek funding, for example
            <li>Blogpost</li>
         ...
         <InputContainer
              ...
              />
         <div>
          {!success  ? <ButtonAlt text='Send verification request'/>
            : <div>Verification request sent</div>}
        </div>
    </Container>
}
