...
const RewardTokenSubmit = ({ add, home, pid, tokenAddress, cap, pledge, tokenAmount }) => {
  ...
  return (
    <ColRight>
      <ButtonBox>
        ...
        <ApproveUniversal tokenContract={tokenAddress} spender={add} amount={cap * tokenAmount } dec={1} />
        {!loading ? <ButtonAlt
          text={'Create reward'}
          onClick={() => {
            handleSubmit();
